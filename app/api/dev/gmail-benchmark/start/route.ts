...
import { serializeBenchmark } from "@/lib/server/live-scan-store";
...
export async function POST(request: Request) {
  try {
    ...
    const accepted = await createGmailBenchmarkSession({
    ...
    return Response.json({ progress: serializeBenchmark(accepted.progress), reused: accepted.reused });
  } catch (error) {
