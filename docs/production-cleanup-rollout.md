...
- Status/Undo: same session/origin/connection/infrastructure/job-ownership checks, but no new-cleanup flag, paid subscription or Stripe-config dependency. Existing services alone determine exact verified-moved ledger eligibility, deadline and restore state. Uncertain targets are never admitted to Undo.
...
Forward billing requires existing `STRIPE_BILLING_MODE`, `STRIPE_SECRET_KEY`, `STRIPE_WEBHOOK_SECRET`, and `STRIPE_SUBSCRIPTION_PRICE_ID`, with a current matching paid entitlement. `STRIPE_BILLING_ENABLED` controls checkout, not existing entitlement or recovery. Keep checkout disabled until the paid service/UI is ready to deliver. No new pricing or scopes.
...
2. Keep `CLEANUP_WORKFLOW_ENABLED=true`, provider production availability, DB, encryption, OAuth credentials and Cron operational. Do not remove subscriptions or require a new payment for recovery.
3. Confirm new start/confirm requests return 503. Confirm status and eligible Undo still work, including a cancelled/expired paid subscription or unavailable Stripe configuration.
