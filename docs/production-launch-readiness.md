...
Configure the one-time credit webhook and test-mode prices using `docs/stripe-billing-setup.md`. No Customer Portal or recurring price is used. Validate signatures, replay, delayed payment, refunds, shared-account linking and failed reconciliation before enabling sales. Existing subscription records require operator review; application deployment does not cancel Stripe subscriptions.
