...
Undo is available only when the job has no unresolved uncertainty, and it targets only exact positively verified moved messages in batches using the cleanup job's same effective size. This includes recovery Undo for the verified-moved ledger when a job stops partway for a known non-ambiguous reason. Any uncertain move or verification blocks all further mutation, including Undo, until the mailbox is inspected and the ambiguity is resolved outside this pass. For every eligible Undo ledger entry, issue an independent move subrequest using the returned moved ID and its encrypted original folder ID. Require each restore response's possibly changed ID, correlate it by request ID, persist it in a dispatched state, then batch-verify those exact restored IDs against their original `parentFolderId` values. Never restore from the pre-move ID, guess a folder, retry a mutation subrequest automatically or permanently delete. Stop new restore batches after uncertainty while retaining truthful aggregate status and recovery context.
