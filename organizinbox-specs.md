...
**Development provider:** Outlook / Hotmail / Microsoft 365 OAuth plus read-only mailbox scanning
...
> **Review selected messages first. Messages that look important are protected. Approved cleanup moves mail to Trash or Deleted Items.**
...
This means provider credentials persist across ordinary navigation until the user explicitly disconnects, separately removes Google authorization, credentials become invalid, or another genuine authentication failure occurs. Normal Disconnect removes Organizinbox's credentials and transient state for that provider; it does not necessarily remove Organizinbox from the connected-app permissions shown by Google. Reports and required scan/cleanup state are stored temporarily in encrypted form, separately from the persistent session and ProviderConnection. Message bodies and attachments are not fetched. Raw Subject lines are processed transiently for protection, then discarded, not persisted. Disconnect removes the temporary restoration state needed for Undo and Recovery Undo; reconnecting does not recover that state.
...
## Recovery And Worker Fencing

Gmail confirmation and Workflow scheduling are separate durable boundaries. Once confirmation is saved, repeating confirmation may re-dispatch the same non-terminal confirmed CleanupJob, including after a scheduling error or an ambiguous scheduler response. Never create another job, reset frozen targets, clear dispatch intent, or repeat a mailbox mutation to recover scheduling. Concurrent confirmations converge through compare-and-set; overlapping Workflow runs still use the existing job lease and exact dispatch/verification ledger. Terminal, unconfirmed, cancelled, expired and restore-in-progress jobs cannot gain new Trash authorization through this recovery path.

Durable scan workers must check their exact scan ID, running status, unexpired state and lease owner before further provider requests. Worker progress/completion writes must atomically enforce the same fencing, including active provider connection and aggregate scan status. A rejected write, lost/expired lease, cancellation, disconnect, deleted/replaced state or inability to establish ownership stops work; it must not be ignored or overwrite the replacement/cancelled state. Participation indexing and experimental IMAP scans have the same checks as main metadata scans. Transport checks do not change classifier evidence.

Disconnect invalidates provider access before clearing transient state. Cached worker tokens do not authorize new requests after durable disconnection: provider request boundaries revalidate the connection and applicable job/scan state. An already-dispatched network operation cannot be recalled; finish/close it safely without dispatching subsequent work. Mutation dispatch/uncertainty rules remain unchanged, including no automatic retry of ambiguous non-idempotent moves. Outlook Recovery Undo is restricted to the exact verified-moved ledger described below. Lease fencing is not a distributed transaction with either provider; a disconnect racing an already-authorized in-flight request cannot guarantee remote cancellation.

Gmail and Outlook cleanup acceptance retry a rolled-back Prisma/Postgres serialization conflict (`P2034`) at most three times before failing closed. Retry only the database transaction that atomically creates the aggregate job and encrypted transient state; never include Workflow scheduling or provider requests in this retry. The unique acceptance key still resolves duplicate submissions to the existing exact job. This handles real serializable write contention, not only duplicate-key (`P2002`) outcomes.

The same bounded database-only serialization retry applies to atomic ScanState/progress writes. A duplicate scan request must not report failure merely because its update to the already-accepted scan encounters a rolled-back serialization conflict. Version/owner/expiry predicates remain unchanged and are re-evaluated on every retry.

Scan progress-only snapshots are coalesced to at most one per five seconds after the first batch. Initial ownership writes, Graph fallback resets, terminal reports and errors bypass coalescing. Every existing provider-request fence and lease check remains in force; partial aggregators were never resumable checkpoints, and replacement scan workers still restart safely. Outlook cleanup retains every frozen-target, dispatch-intent, returned-ID, verification and terminal checkpoint. Its separate ten-minute lease heartbeat is renewed only when at least thirty seconds have elapsed, with renewal failure stopping execution; the lease owner and expiry are still checked on every durable save and provider request.

Browser scan/cleanup status polling is single-flight, abortable on operation replacement/unmount, and scheduled after each response: one second initially, two seconds after ten seconds, and three seconds after one minute. Focus/visibility restoration requests an immediate non-overlapping refresh. Terminal/error responses are applied immediately when received; no server-side terminal/error write is delayed. Without a push transport, long-running operations have up to three seconds of polling detection latency plus request time. Action responses remain immediate.

The Gmail per-message scan identity map is batch-local in lifetime: consume entries during synchronous classification and clear any leftovers after each batch. Retain only the eligible identity records needed to build the exact UID/UIDVALIDITY/API-ID cleanup bridge, then release that intermediate array once final cleanup targets are built. Development diagnostics may expose aggregate current/peak pending identity counts, never the identities themselves.
...
Deepen the existing `/gmail-cleaner`, `/outlook-cleaner`, `/delete-emails-by-sender` and `/delete-old-emails` pages rather than generating new thin pages. Use distinct search-intent titles, useful steps, limitations, privacy/Undo guidance, related internal links and visible FAQs with matching schema. Provider availability stays explicit and must not replace the substance of guides or trust pages. Robots rules apply the same private-route exclusions to every crawler, including the bare `/app` route; robots is not access control. Omit sitemap modification dates when a maintained content-change date is unavailable; never use request/build time as a content update date.
...
The normal secondary Outlook path leads to honest Outlook information while production support remains unavailable. Microsoft connection, scanning and the separately gated 500-message cleanup validation are development-only and are not a public availability signal.
...
The generic public entry flow is:

**Clean my inbox -> Choose provider -> Gmail or Microsoft connection flow**

`/connect` is the provider chooser. It uses the marketing shell and presents Gmail and Outlook as clear responsive options. Gmail continues to `/connect/google`. In non-production development with `MICROSOFT_OAUTH_DEV_ENABLED="true"`, Outlook continues to `/connect/microsoft`; otherwise Outlook remains visibly unavailable/coming soon and must not expose a functional OAuth action.

The chooser and provider pages must not imply that production Outlook support or cleanup is available. They may state that Microsoft connection and scanning are available for development testing. The gated 500-message cleanup validation belongs only inside the authenticated development app, not public provider pages.

After the user selects Gmail:

### Connect Gmail

Outlook users may open an Outlook information page while production support is coming soon. Normal production navigation must not send them into Microsoft OAuth. In non-production development, `MICROSOFT_OAUTH_DEV_ENABLED="true"` may expose a clearly labeled `Connect Microsoft` test path and, after connection, Scan and Report actions. The authenticated report may expose Review Cleanup only when the separate Outlook cleanup gate is also enabled.
...
The implemented OAuth and read-only scan foundation uses:

**Microsoft OAuth + Microsoft Graph**

Use the Microsoft identity platform v2 authorization-code flow as a confidential server application with a fresh signed state, S256 PKCE challenge/verifier and OIDC nonce for every attempt. The default authority is `common`, and the Entra app registration must support accounts in any organizational directory plus personal Microsoft accounts so Outlook.com and Hotmail are not accidentally excluded. A deliberately configured `organizations`, `consumers`, tenant GUID or verified tenant domain may narrow account eligibility, but the configured authority value must be validated before it is placed in an endpoint URL.

The normal Graph connection requests exactly `openid profile email offline_access https://graph.microsoft.com/Mail.ReadWrite`. `openid`, `profile` and `email` establish the signed Microsoft account identity without a Graph profile request; the email claim is optional and connection ownership must remain valid when Microsoft omits it. `offline_access` is required because Organizinbox needs a refresh token for a persistent connection. Delegated `Mail.ReadWrite` is the narrowest Microsoft Graph permission that supports the eventual product requirement to read mail metadata and move messages between the inbox and Deleted Items for both personal and organizational accounts. Do not request `User.Read`, `Mail.Send`, shared-mailbox permissions, application permissions, Files/attachment permissions, directory-wide permissions or `.default`.
...
Use efficient pagination and `$select` to retrieve only fields required by the normalized Organizinbox model. The read-only scan may request message ID, conversation ID, sender/from identity, received date/time, read state, importance, follow-up flag status, categories, Subject for transient protection only, draft state, parent folder ID and `internetMessageHeaders`. Microsoft Graph v1.0 does not define a `size` property on `message`, so Outlook message-size and aggregate recovery estimates remain unavailable rather than requesting an unsupported field or inventing a value. Every Outlook report surface that would otherwise display a zero-byte recovery or size estimate must display `Unavailable`, including overview, sender and category views; storage sorting is unavailable when the provider has no size evidence.

The normal large-mailbox read-only path uses the proven mailbox-wide `/me/messages` collection with one sequential trusted `@odata.nextLink` pagination chain. Each page includes the complete selected classifier evidence inline, including `internetMessageHeaders`, and starts at `$top=100`. Do not partition the normal main scan by folder, run concurrent folder workers, start at `$top=200` or add a selective header-enrichment pass. After an HTTP 413 or a malformed/invalid main-message collection response, discard the complete partial aggregation and restart the mailbox-wide chain at `$top=50`. An exhausted 504 remains a transport failure after bounded retry and must not trigger page-size fallback. Never edit a Graph `@odata.nextLink`, infer an offset, resume a failed page or combine records from failed and fallback attempts.

The folder-partitioned concurrent scanner and its `$top=200` starting size remain development experiments only and disabled by default. They must never be selected by the normal Outlook scan route or production execution. Keep them isolated for controlled comparison until explicitly removed or promoted by a later specification change.

Header access remains required because the provider-neutral classifier uses `List-Id`, `List-Unsubscribe`, `Auto-Submitted` and `Precedence` in per-message classification, sender-level signal unions, dominant category selection and diagnostics. A candidate-only enrichment pass would change those outputs, while enriching every message through 20-item Graph JSON batches would increase a 10,000-message scan to roughly 500 header round trips before metadata requests. Keep `internetMessageHeaders` in the bounded mailbox-wide page selection instead. Process each page transiently, derive only allowlisted header evidence, clear raw page references before yielding normalized records, and discard every raw header. Diagnostics must report zero header-enrichment requests and zero selectively enriched messages for this inline-header path. Do not request body, body preview, attachments, attachment metadata or MIME content. Outlook does not need to use the same transport as Gmail. Provider-specific transport can differ while the application domain model stays provider-neutral.

The mailbox-wide `/me/messages` chain scans mail across folders rather than only the visible Inbox. Independently resolve Inbox, Deleted Items, Drafts and Sent Items through the documented locale-independent well-known folder routes, then traverse read-only folder metadata only to classify each returned message's `parentFolderId` and protect system-folder descendants. Do not request a `wellKnownName` mail-folder property because Microsoft Graph does not define that property on `mailFolder`; well-known names are route aliases, not selectable response fields. Messages whose `parentFolderId` is the exact Deleted Items folder ID or any discovered descendant ID must never be Suggested. A Graph message marked as a draft must also be protected regardless of folder. Unknown or malformed folder evidence fails closed to Protected rather than being treated as cleanup-eligible.

Follow every trusted same-origin Microsoft Graph `@odata.nextLink` until exhaustion. Reject malformed, non-HTTPS or non-Graph continuation URLs. The scan must process pages incrementally with bounded retained message state, expose real checked-message progress, and support 10,000-message mailboxes with a practical path to 50,000. Bounded retry may handle 429 responses using a safe `Retry-After` value plus bounded jitter and transient 5xx failures; 401/403, malformed responses, exhausted retries and pagination failures produce sanitized scan errors without provider response bodies.

Microsoft message delta is folder-scoped and an opaque delta link alone cannot reproduce an exact Organizinbox report: deletions, moves and updates require subtracting each prior message's sender/category/protection contribution before rebuilding aggregates. Until the encrypted transient-state format has a size-bounded per-message contribution ledger and expiry/recovery policy suitable for 50,000-message mailboxes, production scans remain explicit `full` scans. Never apply delta changes to the prior aggregate without that ledger. Future delta links and contribution state, if implemented, must live only in the encrypted transient scan payload, be scoped per folder and connection, fall back to a full scan on expiry/invalidity, and never enter normal Prisma columns, logs, browser URLs or diagnostics.

**Development-only Outlook IMAP read-only benchmark**

When `OUTLOOK_IMAP_BENCHMARK_DEV_ENABLED="true"` outside production, the Outlook scan screen may switch between the existing Graph scanner and an IMAP/XOAUTH2 benchmark against `outlook.office365.com:993`. Graph remains the default and authoritative transport; this benchmark must not replace it or alter cleanup. One scan snapshot records its selected transport, and a duplicate running scan reuses the installed transport rather than changing it mid-run.

The IMAP benchmark uses ImapFlow with TLS certificate verification, XOAUTH2, logging disabled, auto-IDLE disabled and every mailbox opened read-only. Enumerate all selectable folders once. Identify Sent, Drafts and Deleted Items/Trash by IMAP SPECIAL-USE and protect their descendants by hierarchy; if any required protected system role is unavailable, fail the benchmark rather than scanning with incomplete folder protection. Fetch sequence ranges in 250-message batches. The Sent participation index requests only `Thread-Index`, `Message-ID`, `References` and `In-Reply-To`; the main scan requests only UID, internal date, flags and an explicit header allowlist: `From`, `Subject`, `List-Id`, `List-Unsubscribe`, `Auto-Submitted`, `Precedence`, `Importance`, `X-Priority`, `Thread-Index`, `Message-ID`, `References` and `In-Reply-To`. Do not request envelope macros, arbitrary headers, full source, body parts, body structure, body preview, attachments, MIME content or size. Process Subject only through the existing protection detector and discard raw header buffers after each batch.

Map `\\Seen` to read state, `\\Flagged` to the provider-neutral starred hard protection, high `Importance` or priority 1/2 to important protection, folder role to Sent/Draft/Deleted protection, and the existing allowlisted list/automation headers to the same provider-neutral fields used by Graph. Treat non-system IMAP keyword flags conservatively as provider-neutral user labels so labeled mail retains its Review signal, but do not report Graph category evidence as available or infer Promotions, Social, Updates or Personal. IMAP does not expose Graph conversation IDs. Derive only transient SHA-256 thread evidence from the stable root of Exchange `Thread-Index` and normalized `Message-ID`/`References`/`In-Reply-To`; index Sent evidence first, then mark a message participated when any of its derived identifiers intersects that set. Missing usable thread evidence remains unavailable and must never weaken another hard protection. Never persist UIDs, UIDVALIDITY, message/thread identifiers, folder paths, Subjects, headers or sender/message mappings.

The IMAP benchmark diagnostic may include transport, messages scanned, selectable folders scanned, metadata batches, counted IMAP client command round trips, duration, messages per second, aggregate retries/errors, a peak retained-memory estimate, classifier totals and existing safety-invariant counts. It must retain the existing privacy assertions and must not include mailbox paths, UIDs, provider identifiers, Subjects, senders, raw headers, tokens, server responses or command text. Graph diagnostics remain unchanged apart from the explicit `transport: graph` field.

Map Outlook evidence into the existing provider-neutral classifier and aggregate report model. Unavailable Gmail-only labels, headers or category evidence remain unavailable; do not invent equivalents. Outlook importance maps to important protection, Graph `flag.flagStatus=flagged` maps to the provider-neutral starred hard protection, Graph read state maps to unread evidence, conversation identity supports conservative participation/thread evidence where available, and Subject may be processed transiently only through the existing protection detector. A missing or malformed selected flag status is incomplete safety metadata and fails closed. Preserve flagged/starred, recent, important, Sent, Draft, Deleted Items and uncertain-message protection. "When we're unsure, we leave it alone."

Keep the Outlook scan and resulting report in the existing transient report/progress architecture. Do not persist raw messages, message IDs, Subjects, sender mappings, folder mappings or headers in normal Prisma columns. Development diagnostics may record aggregate messages scanned, Graph page/request counts, duration and messages per second, but must never include tokens, message IDs, Subjects, senders, folder names or provider response bodies.

**Durable multi-user scan coordination**

Production Gmail and Outlook scans must not depend on a process-local `globalThis` map or the lifetime of the request that accepted the scan. Keep one database-backed transient scan-state record per user and provider. Its normal columns contain only ownership, provider, opaque scan ID, aggregate lifecycle status, optimistic version, expiring lock/lease ownership, expiry and timestamps. The current progress snapshot, report aggregation, participation identifiers, provider continuation state and any other mailbox-derived scan state belong only in authenticated application-encrypted transient payload storage. Raw Subjects, headers, sender/message mappings, message IDs, conversation IDs, provider continuation URLs and tokens must never become normal Prisma columns or logs.

Scan acceptance is atomic on the user/provider key. When no scan is running, exactly one request may install a new opaque scan ID and begin durable read-only work. A simultaneous request for that same user/provider must return the installed running scan or reject cleanly; it must never start another provider scan. Different users and different providers do not share a lock or mutable application singleton. Workflow input contains only the opaque scan ID. A replacement process loads encrypted progress/state from Postgres, and a retry of read-only scan work may safely restart the scan from the beginning under the same scan ID after clearing partial aggregation; it must never combine partial results from separate attempts. Completed report state remains transient and encrypted until expiry, rescan or Disconnect.

Microsoft Graph work additionally uses a database-backed bounded lease scoped to the exact ProviderConnection. The lease bounds simultaneous scan/cleanup executions for one Microsoft connection while allowing unrelated users to proceed independently. Lease acquisition, refresh, expiry and release are compare-and-set operations; no cross-user in-memory semaphore is permitted. Graph retry delays include bounded jitter so simultaneous throttles do not synchronize retry bursts. This coordination must not change selected metadata, pagination, classification or cleanup safety semantics.

In non-production development, a completed Outlook report may expose a copyable `ORGANIZINBOX DEV OUTLOOK SCAN SUMMARY` for the current transient scan snapshot. It may include aggregate mailbox/classification/protection counts, field-availability flags, scan mode (`full` or a future exact `delta`), folders scanned, total and maximum concurrent Graph requests, metadata requests, header-enrichment requests, messages enriched, main-message page count, the set of effective metadata page sizes, fallback count, 429 wait milliseconds, a peak per-scan retained-memory estimate, Graph 401/403/429/5xx response counts, an aggregate count of other Graph 4xx responses, the last such HTTP status, a status-derived error category and a fixed non-mailbox operation phase such as `folder_resolution`, `conversation_index`, `main_message_scan` or `header_enrichment`, retry counts, timing, throughput, explicit privacy assertions and an aggregate result. These fields must make the active page-size and fallback path unambiguous. Non-HTTP failures may additionally record only a fixed operation phase and fixed category such as `network`, `invalid_json`, `invalid_shape` or `normalization`. The client must not read a provider error response body to classify failures, and diagnostics must not retain successful response content. Replacing the active scan replaces this diagnostic snapshot and resets copy feedback. The copied summary must never contain Microsoft message or conversation IDs, Subjects, sender identities, folder IDs or names, raw headers, `@odata.nextLink` or delta-link URLs, tokens, authorization data, provider error codes or provider response bodies, and it must not be persisted outside the encrypted transient scan snapshot.

After Microsoft connection, `/app` and Account may offer `Scan Outlook inbox`; `/app/scan` shows real Outlook scan progress and routes a completed scan to the existing Inbox Report UI. A current Outlook report offers `View Inbox Report` and `Rescan inbox`. The report identifies Outlook/Microsoft as its provider. It exposes `Review cleanup` only in non-production when the dedicated Outlook cleanup gate is enabled; otherwise cleanup stays unavailable.

**Development-only Outlook cleanup, maximum 500 messages**

Outlook uses the normal Organizinbox flow: `Outlook report -> Review cleanup -> confirm -> move to Deleted Items -> verify -> Undo -> restore -> verify`. This is not a separate proof screen or job subsystem. It reuses the provider-neutral `CleanupJob`, Prisma-backed encrypted `CleanupJobState`, Vercel Workflow coordination, aggregate status model and Review Cleanup workspace already used by durable Gmail cleanup. The Outlook review must use the same user-facing action hierarchy, confirmation structure, working-state treatment and result structure as Gmail wherever provider semantics permit. The destination label remains provider-specific: Gmail uses `Trash` and Outlook uses `Deleted Items`. Provider-specific Graph operations plug into that shared lifecycle. Existing Gmail routes, limits, payloads and behavior remain unchanged.

The flow requires non-production, fixture mode false, the Microsoft development UI gate and a separate default-off `OUTLOOK_CLEANUP_DEV_ENABLED="true"` gate. It accepts an integer requested count from 1 through 500 only, with 5, 10, 25 and 500 exposed as development choices. The Outlook report's Suggested sender groups drive selection. A fresh read-only whole-job safety preflight freezes at most the requested number of exact currently eligible messages from the selected groups before confirmation. If fewer than the requested number can be frozen, fail closed rather than silently widening the selection. Once frozen, never substitute another message. One explicit user confirmation authorizes the complete frozen job; no intermediate batch may require another confirmation.

After explicit confirmation, refresh and persist the participated-conversation and folder safety context inside encrypted transient state, then automatically process every frozen target. Choose one effective Graph batch size from the frozen requested count: 5 messages for jobs up to 25, 10 messages for jobs from 26 through 100, and 20 messages for jobs over 100 through the 500-message maximum. Use that same effective size for final recheck, move, verification, Undo move and Undo verification. Organize progress into 100-message execution chunks, containing twenty, ten or five mutation batches according to the effective size. A Workflow step advances at most one effective-size mutation batch and persists its state before the next durable Workflow iteration. Use one Microsoft Graph JSON batch to re-read every exact target in that batch with the full Graph safety selection, classify every response locally, and persist the approved/excluded decision before submitting that batch's move request. This bounded two-phase shape preserves a final mutable recheck immediately before mutation while limiting the recheck-to-move window; it must never combine a recheck and move in one Graph batch because Graph dependency status cannot enforce Organizinbox's metadata eligibility decision. Normalize and classify with the same folder, flag, importance, conversation, header and Subject-protection rules as scanning. A missing, malformed, newly protected, unknown-location, Sent, Draft, Deleted Items, participated, recent, important, flagged or otherwise ineligible message is excluded by safety; do not replace it. Capture its current original `parentFolderId` only if it passes this final mutable recheck.
...
Production coordination uses transactional version-based compare-and-set plus an expiring lock owner. A worker may claim state only when no unexpired lock exists, and may refresh or release only its own lock. Central lifecycle defaults are a 30-minute active/Undo expiry and a 60-second lock expiry. Refresh state expiry only for legitimate active or user-confirmed transitions. A completed job with Undo available receives a fresh configured Undo-window expiry; the TTL helper adds no separate cleanup buffer. Before deleting `CleanupJobState` after `UNDO_COMPLETE`, atomically persist an aggregate-safe terminal snapshot on the long-lived CleanupJob. That snapshot may contain status, requested/checked/excluded/moved/restored/failed/uncertain counts, completed chunks, safe request/quota totals and timestamps, but never Gmail IDs, UIDs, UIDVALIDITY, history IDs, Subjects, headers, queries or sender/message mappings. Status reads fall back to this terminal snapshot after transient deletion so the browser reaches `UNDO_COMPLETE`, renders final diagnostics and stops polling. Undo completion, expiry and Gmail disconnect still delete the transient row promptly. A `CHUNK_COMPLETE`, `PAUSED`, `PARTIAL`, `FAILED` or conservatively reconciled `UNCERTAIN` cleanup that already has an exact verified-moved ledger may offer a recovery restore for only that ledger while it remains within its bounded TTL; it must not continue mutation after recovery begins and must never present this as full-job Undo. Unprocessed chunks, safety exclusions, failed targets and uncertain attempted targets are excluded. Postgres uses `expiresAt` as source of truth rather than native TTL, with workflow finalization and bounded periodic cleanup responsible for physical deletion. Database unavailability, uncertain lock ownership, version conflict, expired state or encryption failure blocks provider mutation. The normal unit suite keeps the in-memory adapter; production and explicit local integration testing select the Prisma/Postgres adapter through server-only configuration.

Cleanup acceptance itself is also database-atomic. Store only an opaque deterministic acceptance hash on the aggregate CleanupJob and enforce uniqueness with its Scan. Gmail and Outlook must create the aggregate job and encrypted transient state in one transaction. Concurrent requests for the same user/provider/scan/selection/count reuse the one committed job; do not scan an arbitrary newest-job window and do not rely on process-local single-flight state. Jobs with different acceptance hashes or owners remain independent and retain job-scoped locks.

ProviderConnection permits at most one row per user/provider. OAuth callback persistence uses the database compound unique key and a transactional upsert so simultaneous callbacks cannot create duplicates while reconnect still updates/reactivates the existing provider row. Access-token refresh uses a database-backed, expiring per-connection refresh lease plus a monotonic token version. One request exchanges the current refresh token; simultaneous requests wait for and consume the newer committed token version rather than exchanging or overwriting a rotated refresh token themselves. Lease ownership and token writes are scoped to the exact connection and user, and failure releases or expires only that connection's lease.
...
Keep only the transient state needed for the existing Undo/recovery window after cleanup completes. Remove it on lifecycle finalization or expiry under the deletion policy above; cleanup completion does not bypass a still-valid Undo window. Do not retain a permanent Inbox Report result as a database record if it contains user-specific mailbox-derived analytics.
...
```text
No provider:
  generic page -> Clean my inbox -> /connect
  Gmail-specific page -> Clean my Gmail -> /connect/google
...
In non-production development with `MICROSOFT_OAUTH_DEV_ENABLED="true"`, the Outlook landing page may additionally show `Connect Outlook` -> `/connect/microsoft`, accompanied by explicit copy that Microsoft connection and scanning are available for development testing. The public page must not advertise or link the separately gated authenticated 500-message cleanup validation. With the gate disabled, retain the normal coming-soon state.

Provider availability is an explicit server-side product boundary. In production, Gmail requires `GMAIL_PRODUCTION_ENABLED="true"` and Outlook requires `MICROSOFT_PRODUCTION_ENABLED="true"`, plus valid provider OAuth configuration, an HTTPS application/callback URL, PostgreSQL configuration, token and transient-state encryption keys, and retention Cron authentication. Neither credentials alone nor development flags enable a production provider. Missing or invalid required configuration disables the affected provider. Shared missing infrastructure disables both. Availability here permits connection and read-only scanning only; the existing Gmail and Outlook cleanup/proof paths remain development-only and are not promoted to production by these flags.

Production always forces fixture mode off, including when `ORGANIZINBOX_FIXTURE_MODE="true"` was accidentally configured. All development gates are forced off in production. Classifier panels, copyable diagnostics, proof runners, benchmark controls (including Outlook IMAP), scalable validation controls, destructive test routes and development cleanup sizes must neither render nor be actionable in production. API routes and worker/provider request boundaries enforce availability independently of the UI. Normal progress may expose only the minimal aggregate progress required by the product, not development diagnostic payloads.
...
Scan-start failures return sanitized, non-cacheable errors: failure to accept/schedule a durable scan is retryable and must not instruct users to reconnect unnecessarily. Retrying uses the existing atomic scan acceptance/fencing. Operational scan-start events contain only an allowlisted event and provider, never exception text or scan/account identifiers. Deployment checks and alert thresholds are recorded in `docs/production-launch-readiness.md`; its earlier hard-off description is superseded by this rollout policy and `docs/production-cleanup-rollout.md`.
...
Needs reconnect:
  primary Reconnect Gmail or Reconnect Microsoft for the active provider

Microsoft connected in development:
  show Microsoft connected
  no active report: primary Scan Outlook inbox, secondary Account
  active report: primary View Inbox Report, secondary Rescan inbox
  keep public/production Outlook support labeled coming soon
  when OUTLOOK_CLEANUP_DEV_ENABLED is true, the authenticated current report may expose Review cleanup for the bounded 500-message validation
  when the cleanup gate is false, do not expose Cleanup or mutation actions
```
...
`Disconnect Microsoft` follows the same local, confirmed, POST-only and same-origin behavior for an active Microsoft connection. It destroys only the active Microsoft credentials and connection identity, marks that ProviderConnection disconnected, clears the application session and any applicable transient state, and redirects to `/`. It must not revoke Microsoft authorization remotely, affect an existing Gmail ProviderConnection, or expose a provider response.
